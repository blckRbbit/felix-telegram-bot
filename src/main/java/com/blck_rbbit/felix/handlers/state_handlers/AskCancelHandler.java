...
@Slf4j
@Component
public class AskCancelHandler implements InputMessageHandler {
    private final UserDataCache userDataCache;
    private final ReplyMessagesService messagesService;

    public AskCancelHandler(UserDataCache userDataCache, ReplyMessagesService messagesService) {
    ...
    public BotState getHandlerName() {
        return BotState.ASK_CANCEL;
    }

    private SendMessage processUsersInput(Message inputMsg) {
        int userId = inputMsg.getFrom().getId();
        long chatId = inputMsg.getChatId();
        SendMessage replyToUser = messagesService.getReplyMessage(chatId,"reply.askCancel");
